...
interface IncomingMoneyMirrorProps {
  receipts: Receipt[];
  payments: Payment[];
  className?: string;
  hideFooter?: boolean;
...
export function IncomingMoneyMirror({ receipts, payments, className = "", hideFooter = false }: IncomingMoneyMirrorProps) {
  // Combine receipts and paid payments
  const incomingTransactions = [
    ...
    ...payments.filter(p => p.status === 'PAID').map(p => ({
      id: p.id,
      description: p.reference || 'Pagamento Recebido',
      value: p.amount,
  ...
  const maxAmount = Math.max(...incomingTransactions.map(t => t.value), 1);
      ...
      <div className="flex flex-col gap-2 md:gap-3 relative z-10">
        {incomingTransactions.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-2 md:py-4 opacity-30">
            ...
            <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.3em] text-white/50">Sem entradas</span>
          </div>
        )}
      </div>
