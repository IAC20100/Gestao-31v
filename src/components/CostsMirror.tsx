      ...
      <div className="flex flex-col gap-2 md:gap-3 relative z-10">
        {recentCosts.length > 0 ? (
          recentCosts.map((cost, idx) => {
            ...
            return (
              <div key={cost.id} className="flex items-center gap-2 md:gap-3">
                <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                  <DollarSign className="w-3 h-3 md:w-4 md:h-4 text-rose-400" />
                </div>
                <div className="flex-1 flex flex-col gap-0.5 md:gap-1 min-w-0">
                  <div className="flex justify-between items-center text-[8px] md:text-[9px] font-black uppercase tracking-wider text-white/60 gap-2">
                    ...
                    <span className="text-rose-400 shrink-0">R$ {cost.value.toLocaleString('pt-BR')}</span>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-2 md:py-4 opacity-30">
            <TrendingDown className="w-8 h-8 md:w-10 md:h-10 mb-1 md:mb-2 text-white/50" />
            <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.3em] text-white/50">Sem custos</span>
          </div>
        )}
      </div>
      ...
      {!hideFooter && (
        <div className="mt-3 md:mt-4 flex items-center gap-2 md:gap-2.5 px-1 relative z-10">
          <div className="relative shrink-0">
            <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-rose-500 animate-ping absolute inset-0" />
            <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-rose-500 relative" />
          </div>
          <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.3em] text-white/50 truncate">Fluxo de Saída</span>
        </div>
      )}
    </div>
  );
