...
const buildings = [
  { id: 'b1', name: 'Condomínio Alpha', units: 120 },
  { id: 'b2', name: 'Edifício Beta', units: 85 },
  { id: 'b3', name: 'Residencial Gama', units: 200 },
];
...
export default function IotAutomation() {
  ...
  const handleLightChange = (room: keyof typeof lights, value: number) => {
    setLights(prev => ({ ...prev, [room]: value }));
  };
  ...
  const handlePumpToggle = (pump: 'caixa' | 'jardim') => {
    ...
    setPumps(prev => ({ ...prev, [pump]: newState }));
  ...
  const handleAutoToggle = () => {
    ...
    setPumps(prev => ({ ...prev, auto: newState }));
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1400px] bg-gradient-to-br from-[#1a2b4c]/90 to-[#0f172a]/90 backdrop-blur-2xl border border-white/20 rounded-[2rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] p-6 md:p-8 flex flex-col gap-6 overflow-hidden">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/10 pb-6 gap-4">
          ...
          {/* Navigation Icons */}
          <div className="flex items-center gap-6 text-white/60 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto">
            {[
              { id: 'visao-geral', icon: Home, label: 'Visão Geral' },
              { id: 'cisternas', icon: Droplets, label: 'Cisternas' },
              { id: 'iluminacao', icon: Lightbulb, label: 'Iluminação' },
              ...
              <button 
                ...
                className={`flex flex-col items-center gap-1 transition-colors hover:text-white min-w-[70px] ${activeTab === item.id ? 'text-white' : ''}`}
              >
                <item.icon className={`w-6 h-6 ${activeTab === item.id ? 'drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]' : ''}`} />
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-4 py-2">
                  ...
                  <div className="flex-1 flex items-center gap-3 bg-white/5 p-2 rounded-xl border border-white/5 cursor-pointer hover:bg-white/10 transition-colors" onClick={handleToggleAllLights}>
                    <Toggle checked={lights.todas} onChange={() => {}} />
