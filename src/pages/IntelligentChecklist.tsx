        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/10 pb-6 gap-4">
          ...
          <div className="flex items-center gap-6 text-white/60 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto">
            {[
              ...
              { icon: Wrench, label: 'Operacional' },
              { icon: Users, label: 'Moradores' },
              { icon: Settings, label: 'Configurações' },
            ].map((item, idx) => (
              <button 
                key={idx} 
                className={`flex flex-col items-center gap-1 transition-colors hover:text-white min-w-[70px] ${item.active ? 'text-white' : ''}`}
              >
                <item.icon className={`w-6 h-6 ${item.active ? 'drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]' : ''}`} />
